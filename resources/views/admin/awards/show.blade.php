                ...
                <div class="col-md-6">
                    <table class="table table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <td field-key='docsrc's> @foreach($award->getMedia('docsrc') as $media)
                                <p class="form-group">
                                    <a href="{{ $media->getUrl() }}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
                                </p>
                            @endforeach</td>
...
<li role="presentation" class="active"><a href="#projects" aria-controls="projects" role="tab" data-toggle="tab">Projekty</a></li>
            ...
            @foreach ($projects as $project)
                <tr data-entry-id="{{ $project->id }}">
                    <td field-key='name'>{{ $project->name }}</td>
                                <td field-key='imgsrc'>@if($project->imgsrc)<a href="{{ asset(env('UPLOAD_PATH').'/' . $project->imgsrc) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/' . $project->imgsrc) }}"/></a>@endif</td>
...
{!! Form::open(array(
        ...
        @endif
            ...
            <a href="{{ route('admin.awards.index') }}" class="btn btn-default">@lang('global.app_back_to_list')</a>
        </div>
    </div>
@stop
